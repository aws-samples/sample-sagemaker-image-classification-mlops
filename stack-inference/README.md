...
6. Schedules two Processing jobs via EventBridge Scheduler: prediction drift (PSI) and subgroup fairness (Fairlearn)
...
| monitoring\_job\_image\_tag | Tag of the AWS-managed scikit-learn Processing image used to run the scheduled drift and fairness scripts. | `string` | `"1.2-1"` | no |
...
├── monitoring.tf               # Drift + fairness schedules, alarms, retrain rule, Bedrock guardrail
