...
echo -e "${BLUE}⬇️  Downloading BreakHis (~4GB, may take several minutes)...${NC}"
if [ -f "data/${TARBALL}" ]; then
    echo -e "${YELLOW}ℹ️  Found cached tarball at data/${TARBALL}, reusing${NC}"
...
echo -e "${BLUE}🗂️  Organizing into ${OUTPUT_DIR}/{breast_benign,breast_malignant}/...${NC}"
...
for src in $(find "${BREAKHIS_ROOT}/breast/benign" -type f -iname "*.png"); do
...
for src in $(find "${BREAKHIS_ROOT}/breast/malignant" -type f -iname "*.png"); do
...
echo -e "   1. Validate independently (cross-dataset eval):"
echo -e "      python analysis-scripts/data_cross_dataset_eval.py --data-dir ${OUTPUT_DIR}"
echo ""
echo -e "   2. Or add to training pool (triggers pipeline retraining):"
